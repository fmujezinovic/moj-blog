...
export default function NewPostPage() {
  const f = usePostForm();        // slug je undefined → isEdit = false
    ...
    <main className="flex justify-center px-4 py-10">
      <Card className="w-full max-w-6xl p-8">
        ...
        <CardHeader className="space-y-6">
          <CardTitle className="text-3xl font-bold">Kreiraj Novi Post</CardTitle>
          ...
          <div className="flex items-center gap-3 mt-6">
            <Switch checked={f.published} onCheckedChange={f.setPublished} id="published-switch" />
        ...
        <CardContent className="flex flex-col gap-8">
          <Tabs value={f.selectedTab} onValueChange={f.setSelectedTab}>
            <div className="flex items-center justify-between mb-6">
              ...
              <Button variant="outline" size="sm" onClick={f.addSection}>Dodaj sekciju</Button>
            </div>
          ...
          <div className="flex justify-end gap-4">
            <Button variant="outline" onClick={() => history.back()}>Prekliči</Button>
            <Button onClick={f.save} disabled={f.loading}>Spremi Post</Button>
          </div>
        </CardContent>
      </Card>
    </main>
